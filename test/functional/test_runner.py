...
`test/functional/test_framework/test_framework.py:ColomboTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/colombo_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_colombod = config["components"].getboolean("ENABLE_CLMD")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # http://github.com/colombiacoin/colombo/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # http://github.com/colombiacoin/colombo/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_colombod):
        print("No functional tests to run. Wallet, utils, and colombod must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if colombod is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "colombod"]) is not None:
            print("%sWARNING!%s There is already a colombod process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "CLMD" not in os.environ:
        os.environ["CLMD"] = build_dir + '/src/colombod' + exeext
        os.environ["CLMCLI"] = build_dir + '/src/colombo-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `colombo-cli help` (`rpc_interface.txt`).
