...
The RPM spec file provided here is for Colombo-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://colombo.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Colombo git repository](http://github.com/colombiacoin/colombo) but are not part of
...
Currently only `Source10` is used, for the example `colombo.conf` file.
...
in the colombo git repository. At present only an SVG version of the Colombo
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the colombo
...
needed when 0.12.1 is released, a proper fix is already in the Colombo git
master branch.
...
idea to build Colombo Core against a static version of that library compiled
...
The `colombo.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `colombo-server` package. At this point, most RPM based
...
The `colombo-server` package should have SELinux support. How to properly do
...
Colombo-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/colombo/). He has been
packaging Colombo for Fedora at least since 2012.
...
4. I renamed the `colombo` package that contains the Qt GUI to `colombo-core` as
that appears to be how the general population refers to it, in contrast to
`colombo-xt` or `colombo-classic`. I wanted to make sure the general population
