...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx)
{
    ...
    if (nHeight <= DEV_FUND_UNTIL) {
      CColomboAddress address(DEV_ADDRESS); //hello
      coinbaseTx.vout.resize(2);
      coinbaseTx.vout[1].scriptPubKey = GetScriptForDestination(CColomboAddress(address).Get());
...
void static ColomboMiner(const CChainParams& chainparams)
{
    LogPrintf("ColomboMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("colombo-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("ColomboMiner -- Wallet not available\n");
    }
        ...
        while (true) {
            ...
            if (chainparams.MiningRequiresPeers()) {
                ...
                do {
                    ...
                    LogPrintf("ColomboMiner -- waiting for connections and blocks downloaded\n");
                    MilliSleep(1000);
                } while (true);
            }
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("ColomboMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("ColomboMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ColomboMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("ColomboMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateColombos(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&ColomboMiner, boost::cref(chainparams)));
    }
