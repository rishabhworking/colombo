 ...
 * between colombod, and colombo-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    FlushWallets();
#endif
    GenerateColombos(false, 0, Params());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), CLM_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<http://github.com/colombiacoin/colombo>";
    const std::string URL_WEBSITE = "<https://colombo.org>";
...
/** Sanity checks
 *  Ensure that Colombo is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Colombo process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", CLM_CONF_FILENAME)).string());
    ...
    GenerateColombos(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
