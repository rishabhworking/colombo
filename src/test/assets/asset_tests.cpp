...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of CLMCOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("CLM", type));
        BOOST_CHECK(!IsAssetNameValid("CLM", type));
        BOOST_CHECK(!IsAssetNameValid("CLMCOIN", type));

        //- Versions of CLMCOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("CLM.COIN", type));
        BOOST_CHECK(IsAssetNameValid("CLM_COIN", type));
        BOOST_CHECK(IsAssetNameValid("CLMSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERCLM", type));
        BOOST_CHECK(IsAssetNameValid("CLMSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDECLM", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_CLMS", type));
        BOOST_CHECK(IsAssetNameValid("SECLMOT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("CLM", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
