...
#include "colombogui.h"

#include "colombounits.h"
...
const std::string ColomboGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString ColomboGUI::DEFAULT_WALLET = "~Default";

ColomboGUI::ColomboGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void ColomboGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Colombo address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and colombo: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to CLM addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Colombo addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Colombo addresses"));
    ...
    openAction->setStatusTip(tr("Open a colombo: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Colombo command-line options").arg(tr(PACKAGE_NAME)));
...
void ColomboGUI::createToolBars()
{
    if(walletFrame)
    {
        /** CLM START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/colombotext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* colomboLabelLayout = new QVBoxLayout(toolbarWidget);
        colomboLabelLayout->addWidget(label);
        colomboLabelLayout->addWidget(toolbar);
        colomboLabelLayout->setDirection(QBoxLayout::TopToBottom);
        colomboLabelLayout->addStretch(1);
...
void ColomboGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool ColomboGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool ColomboGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void ColomboGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ColomboGUI::setWalletActionsEnabled(bool enabled)
{
...
void ColomboGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void ColomboGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void ColomboGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ColomboGUI::optionsClicked()
{
...
void ColomboGUI::showDebugWindow()
{
...
void ColomboGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void ColomboGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void ColomboGUI::openClicked()
{
...
void ColomboGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ColomboGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ColomboGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ColomboGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ColomboGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ColomboGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** CLM START */
void ColomboGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void ColomboGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void ColomboGUI::gotoManageAssetsPage()
{
...
void ColomboGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Colombo network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void ColomboGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ColomboGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void ColomboGUI::updateHeadersSyncProgressLabel()
{
...
void ColomboGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void ColomboGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Colombo"); // default title
...
void ColomboGUI::changeEvent(QEvent *e)
{
...
void ColomboGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ColomboGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ColomboGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "CLM")
        msg += tr("Amount: %1\n").arg(ColomboUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(ColomboUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void ColomboGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ColomboGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ColomboGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ColomboGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ColomboGUI::setHDStatus(int hdEnabled)
{
...
void ColomboGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ColomboGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ColomboGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ColomboGUI::detectShutdown()
{
...
void ColomboGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void ColomboGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void ColomboGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ColomboGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void ColomboGUI::subscribeToCoreSignals()
{
...
void ColomboGUI::unsubscribeFromCoreSignals()
{
...
void ColomboGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<ColomboUnits::Unit> units = ColomboUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ColomboUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(ColomboUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (ColomboUnits::Unit u : ColomboUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(ColomboUnits::name(u)), this);
...
{
    setText(ColomboUnits::name(newUnits));
}
...
void ColomboGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=CLMBTC"));
