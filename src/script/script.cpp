...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** CLM START */
    case OP_CLM_ASSET              : return "OP_CLM_ASSET";
    /** CLM END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_CLM_ASSET) { // OP_CLM_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == CLM_R) { // Check to see if CLM starts at 27 ( this->size() < 105)
                if ((*this)[28] == CLM_V)
                    if ((*this)[29] == CLM_N)
                        index = 30;
            } else {
                if ((*this)[28] == CLM_R) // Check to see if CLM starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == CLM_V)
                        if ((*this)[30] == CLM_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == CLM_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == CLM_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == CLM_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == CLM_R) {
