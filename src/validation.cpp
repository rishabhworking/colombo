...
#if defined(NDEBUG)
# error "Colombo cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Colombo Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s CLM additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if(nHeight <= 525600)
    {
       nSubsidy = 80 * COIN; // 42048000 + 4204800 developer fund (Line: 5512)
    }
    else if(nHeight <= 1051200)
    {
       nSubsidy = 100 * COIN; // 52560000
    }
    else if(nHeight <= 1576800)
    {
       nSubsidy = 50 * COIN; // 26280000
    }
    else if(nHeight <= 2102400)
    {
       nSubsidy = 40 * COIN; // 21024000
    }
    else if(nHeight <= 2628000)
    {
       nSubsidy = 30 * COIN; // 15768000
    }
    else if(nHeight <= 3679200)
    {
       nSubsidy = 20 * COIN; // 21024000
    }
    else if(nHeight <= 5256000)
    {
       nSubsidy = 10 * COIN; // 15768000
    } 
    return nSubsidy;
}
...
void UpdateCoins(const CTransaction& tx, CCoinsViewCache& inputs, CTxUndo &txundo, int nHeight, uint256 blockHash, CAssetsCache* assetCache, std::pair<std::string, CBlockAssetUndo>* undoAssetData)
{
    ...
    if (!tx.IsCoinBase()) {
        ...
        for (const CTxIn &txin : tx.vin) {
            ...
            bool is_spent = inputs.SpendCoin(txin.prevout, &txundo.vprevout.back(), assetCache); /** CLM START */ /* Pass assetCache into function */ /** CLM END */
            assert(is_spent);
        }
    }
    // add outputs
    AddCoins(inputs, tx, nHeight, blockHash, false, assetCache, undoAssetData); /** CLM START */ /* Pass assetCache into function */ /** CLM END */
}
...
//                                      << ", " << i << ", " << hash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
        ...
        for (size_t o = 0; o < tx.vout.size(); o++) {
            if (!tx.vout[o].scriptPubKey.IsUnspendable()) {
                ...
                Coin coin;
                bool is_spent = view.SpendCoin(out, &coin, &tempCache); /** CLM START */ /* Pass assetsCache into the SpendCoin function */ /** CLM END */
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                int res = ApplyTxInUndo(std::move(undo), view, out, assetsCache); /** CLM START */ /* Pass assetsCache into ApplyTxInUndo function */ /** CLM END */
...
//                                      << ", " << i << ", " << txhash.GetHex() << ", " << k << ", " << "true" << ", " << assetAmount << std::endl;
    ...
    if (nBlockHeight <= DEV_FUND_UNTIL) {
      ...
      CColomboAddress address(DEV_ADDRESS);
      CScript scriptPubKey = GetScriptForDestination(CColomboAddress(address).Get());
...
bool static FlushStateToDisk(const CChainParams& chainparams, CValidationState &state, FlushStateMode mode, int nManualPruneHeight) {
    ...
    try {
    {
        ...
        if (fDoFullFlush) {
            ...
            if (!CheckDiskSpace((48 * 2 * 2 * pcoinsTip->GetCacheSize()) + assetDirtyCacheSize * 2)) /** CLM START */ /** CLM END */
                return state.Error("out of disk space");
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
...

    {
        ...
        LogPrint(BCLog::BENCH, "  - Flush CLM: %.2fms [%.2fs (%.2fms/blk)]\n", (nTime4 - nTime3) * MILLI, nTimeFlush * MICRO, nTimeFlush * MILLI / nBlocksTotal);
...
bool TestBlockValidity(CValidationState& state, const CChainParams& chainparams, const CBlock& block, CBlockIndex* pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (!ConnectBlock(block, state, &indexDummy, viewNew, chainparams, &assetCache, true)) /** CLM START */ /*Add asset to function */ /** CLM END*/
